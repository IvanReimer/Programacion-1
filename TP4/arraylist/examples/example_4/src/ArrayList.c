...
int al_clear(ArrayList* pList)
{
    ...
    {
        returnAux=0;
        ArrayList *pAux=realloc(pList->pElements,sizeof(void*)*AL_INITIAL_VALUE);
        if(pAux!=NULL)
        {
            pList->size=0;
            free(pList->pElements);
...
{
    int returnAux = -1;
    if(pList!=NULL && pElement!=NULL && index>=0 && index<=pList->size && !expand(pList,index))
    {
        pList->pElements[index]=pElement;
        returnAux=0;
        pList->size++;
...
{
    int returnAux = -1;
    if(pList!=NULL && index>=0 && index<=pList->size && (!resizeUp(pList)) )
    {
        int i;
